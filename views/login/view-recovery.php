...
$form -> setHtmlParams("captcha", 'placeholder="'.I18nlocale("captcha").'"');
...
if(HttpisPostRequest())
{
    ...
    if($form -> captcha && $_SESSION["captcha"] != $form -> captcha)
    	$form -> addError(I18nlocale("wrong-captcha"), "captcha");
         
    if($form -> isSubmitted() && $form -> isValid())
		if(!$mv -> accounts -> countRecords(array("email" => $form -> email, "active" => 1)))
			$form -> addError(I18nlocale("email-wrong-blocked"), "email");
...
?>
	<div id="content">
		<div class="registration-block recovery">
			<h1><?php echo I18nlocale("password-restore"); ?></h1>
			<p><?php echo I18nlocale("enter-account-email"); ?></p>
            <?php 
	            if(isset($_GET['done']))
	            	echo "<div class=\"success\"><p>".I18nlocale("change-password-sent")."</p></div>\n";
	            else if(isset($_GET['wrong-token']))
	            	echo "<div class=\"form-errors\"><p>".I18nlocale("password-not-confirmed")."</p></div>\n";
            ...
            <form class="regular" method="post" action="<?php echo $mv -> root_path; ?>recovery">
	            ...
	            <div class="form-buttons clearfix">
	               ...
	               <input type="submit" value="<?php echo I18nlocale("restore"); ?>" class="button big"/>
	            </div>
	            <div class="clear"></div>
            </form>
            <?php endif; ?>
