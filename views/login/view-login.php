...
$form -> addRule("login", "required", true, I18nlocale("complete-login"));
$form -> addRule("password", "required", true, I18nlocale("complete-password"));
...
$form -> setHtmlParams("login", 'placeholder="'.I18nlocale("login").'"');
$form -> setHtmlParams("password", 'placeholder="'.I18nlocale("password").'"');
	...
	$form -> submit() -> validate(array("login", "password"));
   	...
   	if($form -> isSubmitted() && $form -> isValid())
   		if(!$account = $mv -> accounts -> login($form -> login, $form -> password))
   			$form -> addError(I18nlocale("login-failed"));
   		else
   		{
   			if($form -> remember_me)
   			{
   			...
   			{
   				$account -> date_registration = I18ngetCurrentDateTime();
   				$account -> update();
   			}
...
?>
   <div id="content">
      ...
      <div class="registration-block">
         <h1><?php echo I18nlocale("authorization"); ?></h1>
         <?php 
         	echo $form -> displayErrors();
         	
         	if(isset($_GET['recovered']))
         		echo "<div class=\"success\"><p>".I18nlocale("password-confirmed")."</p></div>";
         ?>
         <form class="regular" method="post" action="<?php echo $mv -> root_path; ?>login">
            ...
            <p class="remember-me">
                ...
                <label for="remember_me"><?php echo I18nlocale("remember-login"); ?></label>
            </p>
            <div class="form-buttons clearfix">
               <?php echo $form -> displayTokenCSFR(); ?>
               <input type="submit" value="<?php echo I18nlocale("login-action"); ?>" class="button big"/>
            </div>
            <div class="clear"></div>
            <p class="recovery">
               <a href="<?php echo $mv -> root_path; ?>recovery"><?php echo I18nlocale("forgot-password"); ?></a>
            </p>
         </form>
      </div>
      <div class="clear"></div>
